...
category_df = pd.read_csv('naver_crawler.csv')
...
for catid in range(len(category_df)):
    ...
    # category_nm = category_df['category'][catid]
    category_nm = category_df['대분류'][catid]
    category_nm_middle = category_df['소분류'][catid]
    
    category_code = str(category_df['cate_url_middle'][catid])

    # url = 'https://search.shopping.naver.com/best100v2/detail.nhn?catId=' + category_code
    url = str(category_df['cate_url_middle'][catid])
    ...
    # for i in range(len(content)):
        ...
        df = pd.DataFrame({
            ...
            'category_mid' : [category_nm_middle],
            'title' : [title_name],
            'price' : [price_num],
        })
...
# 저장
total.to_csv("top_100_2.csv", index = False)
